...
template <class Key, class Data> int
InefficientMap <Key, Data> :: IsThereJoinRecord (Key &findMe, OrderMaker &comparator) {
	...
	while (container.RightLength() != 0) {
		Record left = container.Current().key.GetData();
		Record right = findMe.GetData();
		if (comparator.Run(left, right, comparator) == 0)
			return 1;
		container.Advance();
	}
...
template <class Key, class Data> int
InefficientMap <Key, Data> :: RemoveJoinRecord (Key &findMe, Key &putKeyHere,
		Data &putDataHere, OrderMaker &comparator) {
